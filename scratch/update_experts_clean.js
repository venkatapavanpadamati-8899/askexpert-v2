...
// 1. Add CSS rules inside <style> right before </style>
const customCss = `
...
// 2. Replace Controls Panel & Grid in HTML Body
const oldControlsMarkerStart = html.indexOf('<!-- Controls Panel: Search & Filters -->');
const oldGridMarkerEnd = html.indexOf('</section>', html.indexOf('id="expertsGrid"')) + '</section>'.length;

if (oldControlsMarkerStart === -1 || oldGridMarkerEnd === -1) {
  console.error("Could not find controls panel HTML section!");
  process.exit(1);
}

const newControlsHtml = `<!-- Controls Panel: Search, Categories & Filters -->
...
html = html.substring(0, oldControlsMarkerStart) + newControlsHtml + html.substring(oldGridMarkerEnd);

// 3. Replace JS logic inside <script type="module">
const jsMarkerStart = html.indexOf('let allExperts = [];');
const jsMarkerEnd = html.lastIndexOf('initDirectory();');

if (jsMarkerStart === -1 || jsMarkerEnd === -1) {
  console.error("Could not find script section markers!");
  process.exit(1);
}

const newScriptLogic = `let selectedDomain = "all";
    let currentPage = 1;
    const pageSize = 12;
...
html = html.substring(0, jsMarkerStart) + newScriptLogic + html.substring(jsMarkerEnd + 'initDirectory();'.length);

fs.writeFileSync('experts.html', html);
console.log('Clean replacement applied to experts.html successfully!');
